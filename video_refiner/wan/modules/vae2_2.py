...
class WanVAE_(nn.Module):
    ...
    def decode(self, z, scale, out_device=None):
        """Decode latent -> pixel.

        Args:
            out_device: when set (e.g. ``'cpu'``), each decoded temporal chunk is
                moved to this device before being concatenated, so the growing
                output does not accumulate on the (GPU) compute device. This cuts
                peak memory for high-resolution decode. ``None`` (default) keeps
                the original behaviour (accumulate on the compute device).
        """
        self.clear_cache()
        if isinstance(scale[0], torch.Tensor):
